...
# adding these changes to incorperate both GOES 16 and 18
...
# Filename example:
# OR_ABI-L1b-RadC-M6C13_G18_s20230390300211_e20230390309519_c20230390309587.nc
FILENAME_RE = re.compile(
    r"OR_ABI-L1b-RadC-(M\d)(C\d{2})_(G16|G18)_"
...
def hourly_targets(date_str: str, start_z: str, end_z: str) -> list[datetime]:
    ...
    # Handle cases that continue past 00Z into the next day
    if end_dt < start_dt:
...
#def build_hour_prefix(dt: datetime) -> str:
    #year = dt.strftime("%Y")
    #jday = dt.strftime("%j")
    #hour = dt.strftime("%H")
    #return f"{BUCKET}/{PRODUCT}/{year}/{jday}/{hour}/"
...
#def find_nearest_file_for_channel(target: datetime, channel: str) -> #tuple[datetime, str] | None:
...
def find_nearest_file_for_channel(
    ...
    #prefix = build_hour_prefix(target)
...
def main() -> None:
    ...
    for i, row in enumerate(rows, start=1):
        ...
        print(f"\nProcessing Case {case_id}: {date_str} {start_z}-{end_z}Z | {satellite}")
